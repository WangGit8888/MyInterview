...
#### 传统for循环和Stream.forEach区别

| 对比维度     | 传统for循环                  | Stream.forEach                          |
| -------- | ------------------------ | --------------------------------------- |
| **迭代器**  | 自己维护索引或迭代器               | 由Spliterator内部管理                        |
| **执行方式** | **同步串行**，主线程执行           | 可以是**串行**，也支持**并行**（`parallelStream()`） |
| **中断控制** | 支持 `break`、`return` 提前退出 | **不支持**，无法跳出整个循环（只能用`return`跳出当前元素）     |
| **异常处理** | try-catch包裹整个循环体         | 异常处理需在Lambda内部处理，更麻烦                    |
