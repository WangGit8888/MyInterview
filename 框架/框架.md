...
| ---------- | --------------------------- | ----------------------- |
| **所属阵营**   | Spring 框架                   | Java 标准（JSR-250）        |
| **注入顺序**   | **优先按类型（byType）**           | **优先按名称（byName）**       |
| **匹配方式**   | 类型匹配 → 如果多个则用名称（@Qualifier） | 名称匹配 → 名称找不到则按类型        |
| **是否必需**   | required 属性可配（默认true）       | 没有 required 属性，默认必需     |
| **支持的场景**  | 构造器/方法/参数/字段                | 字段/方法（不支持构造器注入）         |
| **需要额外依赖** | 需引入 spring-beans            | JDK自带（javax.annotation） |

### 反射原理

1. 每个类被加载后，JVM会在元空间生成一个唯一的`Class`对象，它存储了类的完整元数据，反射就是通过操作这个对象来获取类信息。

2. 底层依赖JNI调用JVM的native方法完成实际的方法调用和字段访问，而`setAccessible(true)`可以绕过访问控制。

3. 性能确实比普通调用慢，主要原因是无法JIT内联、每次都要做安全检查和方法解析。我之前的项目中，如果高频反射调用，会用缓存`Method`对象来优化。

在实际生产中，反射主要用在框架的依赖注入、ORM映射和注解解析中，我们也会注意不要在大循环里频繁调用反射，避免性能陷阱

真正“每次新建”的是 `Method`、`Field`、`Constructor` 这些**句柄对象**，而不是底层元数据。

### AOP原理

**第一，动态代理层面**。Spring AOP 基于动态代理实现，有接口时默认用 JDK 动态代理，没接口时用 CGLIB。Spring Boot 2.x 开始默认强制用 CGLIB。

**第二，Bean 创建流程层面**。Spring 在 Bean 实例化后的初始化阶段，会通过 `AbstractAutoProxyCreator` 扫描当前 Bean 是否匹配切面表达式，如果匹配，就用 `ProxyFactory` 生成代理对象并返回，容器里最终存的是代理对象而非原始对象。

**第三，调用流程层面**。调用代理方法时，会把所有匹配的增强封装成 `MethodInterceptor` 拦截器链，按顺序递归执行，形成 `@Around` → `@Before` → 目标方法 → `@AfterReturning` → `@After` 的执行顺序。
