...
- `getBean()`最终都是从这里获取，去掉它容器就失去了管理单例的能力
    
- 无法区分"Bean是否已完整初始化"
    

##### 情况2：去掉二级缓存 → ⚠️ 特定条件下可以

- 如果**系统没有任何AOP代理**（没有`@Transactional`、`@Async`等），去掉二级缓存，只用一级+三级也能跑通循环依赖
    
- 但如果存在AOP+循环依赖，去掉二级缓存会导致**代理对象被创建两次**，B引用的代理和容器最终的代理不是同一个对象，导致事务失效
    

##### 情况3：去掉三级缓存 → ⚠️ 特定条件下可以

- 如果**系统没有任何循环依赖**，去掉三级缓存，只用一级+二级也能跑通
    
- 但只要有循环依赖，就必须用三级缓存来**提前暴露尚未初始化的Bean引用**
