...
1. 类信息
    类元数据，包括类名、父类、接口、字段、方法的字节码等
2. 运行时常量池

| 类型                            | 内容                      | 示例                                       |
| ----------------------------- | ----------------------- | ---------------------------------------- |
| **字面量（Literals）**             | 基本数据类型常量、字符串常量、final 常量 | `int a = 10;` `String s = "hello";`      |
| **符号引用（Symbolic References）** | 类名、方法名、字段名，类加载时解析为直接引用  | `invokevirtual #5` 指向 `java/lang/System` |
